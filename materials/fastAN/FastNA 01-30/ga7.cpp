...
int find_node_to_match(int i, int phe_flag) {
    
	current_ant.fit = -1;
	current_ant.nn = 1;
    current_ant.nodeList[0] = -1;
                                 
    for (int j=0; j<g2.n; j++)
    if (current_ant.mark2[j]==0) 
	if ((phe_flag == 0) || (randomdouble() <= pheromone[j])){
		...
		if (numRelation + similar[i][j] > current_ant.fit ) {
		   current_ant.maxRelation = numRelation;
		   current_ant.fit = numRelation + similar[i][j];
...
void ant_k_construct_solutions() {
     ...
     for (k=0; k<g1.n; k++) {
         ...
         j = find_node_to_match(i, ((kAlg & 2) > 0));
		 
		 if (j == -1) 
			j = find_node_to_match(i, 0);
...
void TLAS_pheromone_trail_update(int n_try){
	...
	for (i=0; i<g1.n; i++) {
		...
		pheromone[j] = rho*pheromone[j] + (1-rho)*trail_max;
		pheromone_mark[j] = 2;
	}
	...
	for (j=0; j<g2.n; j++) {
		if (pheromone_mark[j]==1) {

			pheromone[j] = rho*pheromone[j] + (1-rho)*trail_mid;

		}
		else if (pheromone_mark[j]==0){
			pheromone[j] = rho*pheromone[j] + (1-rho)*trail_min;
		}
    }
}
