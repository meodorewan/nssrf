...
void set_parameters() {
     ...
     if (n_ants==-1)		n_ants		= 100;
	 if (alpha==-1)			alpha		= 1.0;
	 if (beta==-1)			beta		= 1.0;
	 if (rho==-1)			rho			= 0.5;
	 if (ls_flag==-1)		ls_flag		= 2;
	 if (kAlg==-1)		    kAlg		= 3;
...
bool local_search(ant_struct &ls_ant) {
    ...
    for (int i=0; i<g1.n-1; i++) {
        ...
        for (int j=i+1; j<g1.n; j++) {
            ...
            int removeEdgI = 0;
            for (int k=0; k<g1.adj[i].size(); k++) 
                removeEdgI -= matrixGraph2[ls_ant.tour[i]][ls_ant.tour[g1.adj[i][k].first]];
            
            int removeEdgJ = 0;
            for (int k=0; k<g1.adj[j].size(); k++) 
                removeEdgJ -= matrixGraph2[ls_ant.tour[j]][ls_ant.tour[g1.adj[j][k].first]];
                
            swap(ls_ant.tour[i],ls_ant.tour[j]);
            ...
            if (removeEdgI + removeEdgJ > 0) {
                 ls_ant.tour_length += removeEdgI + removeEdgJ;            
                 ok = true;
            }
            else swap(ls_ant.tour[i],ls_ant.tour[j]);
...
void construct_solutions(int n_try){
	...
	for (ak=0; ak<n_ants; ak++) {
		...
		cout<<current_ant.tour_length<<endl;
...
*/
	...
	if (ibest_ant.tour_length > best_so_far_ant[n_try].tour_length) {
		...
		cout<<"Iteration "<<setw(5)<<iteration
						  ...
						  <<" -> Best : "<<best_so_far_ant[n_try].tour_length<<endl;
...
int main(int argc, char *argv[])
{
	...
	for ( n_try = 0 ; n_try < max_tries ; n_try++ ) {
		...
		while ( !termination_condition(n_try) ) {
			...
			if ((kAlg & 2) == 1) TLAS_pheromone_trail_update(n_try);
