...
int kAlg;
// 0. -graphlet, -pheromone
// 1. +graphlet, -pheromone
// 2. -graphlet, +pheromone
// 3. +graphlet, +pheromone
...
graph g1, g2;
double *GraphletSimilar[MAX_NODE];        // thong tin heuristic dua tren graphlet
...
void print_parameters_log(){
	 ...
	 flog<<"algorithm                      : "<<kAlg<<"("
											  <<((kAlg & 1)?"+graphlet;":"-graphlet;")
											  <<((kAlg & 2)?"+pheromone;":"-pheromone;")
											  <<")"<<endl;
...
double calGraphletSimilar(int i, int j) {
...
void preprocess_phase() {
    ...
    for (i=0; i<g1.n; i++) {
        ...
        GraphletSimilar[i] = new double[g2.n];
	...
	for (i=0; i<g1.n; i++) {
        ...
        for (j=0; j<g2.n; j++) {
			// su dung graphlet
			if ((kAlg & 1) > 0)
				GraphletSimilar[i][j] = calGraphletSimilar(i,j);
			// khong su dung graphlet
			else GraphletSimilar[i][j] = 1.0;
...
int find_node_to_match(int i) {
    ...
    if (current_ant.mark2[j]==0) {
        ...
        double tmp =  pheromone[i][j] * (alpha * numRelation + (1-alpha) * BlatSimilar[i][j] * GraphletSimilar[i][j]) ;
